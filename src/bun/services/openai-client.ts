import type { FailureCategory, SanitizedProviderError, SubmitRunInput } from "../../shared/contracts";
...
export function sanitizedError(error: unknown): SanitizedProviderError {
  ...
  return {
    message: error instanceof Error ? error.message.slice(0, 240) : "The provider request failed.",
    category: "network",
    httpStatus: null,
    requestId: null,
    retryAt: null,
...
export class OpenAIClient {
  ...
  private async request<T>(path: string, init: RequestInit = {}, timeoutMs = SHORT_TIMEOUT_MS): Promise<{ data: T; requestId: string | null }> {
    ...
    if (!response.ok) {
      ...
      } catch { /* raw provider bodies are deliberately discarded */ }
    ...
    return { data: text ? JSON.parse(text) as T : undefined as T, requestId };
  }
